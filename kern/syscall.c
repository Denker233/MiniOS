...
sys_exofork(void)
{
	...
	child->env_tf.tf_regs.reg_eax = 0;		//child process的返回值应该为0
	return child->env_id;
}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	struct Env *env;
	int r;
	if ((r = envid2env(envid, &env, 1)) < 0) return -E_BAD_ENV;
	tf->tf_eflags |= FL_IF;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if (state1 < 0) return -E_BAD_ENV;
	if (state2 < 0) return -E_BAD_ENV;
